							   ...
							   CACHE_TYPE cache_type)
{
	...
	char ram_line[9];
	ram_line[8] = 0;
	pcb_table.incCacheUsed(pid);//accounting
...
bool cpu::checkCache(int index, CACHE_TYPE cache_type, int pid){
	...
	{
		pcb_table.incCacheMissCount(pid);//accounting
	...
	{
		pcb_table.incCacheHitCount(pid);//accounting
...
int cpu::processIo(const unsigned int& instr){
	...
	switch(opcode){
		case RD:
			pcb_table.incIoReadCount(current_pid);
			if(reg_2==0){
			...
			break;
		case WR:
			pcb_table.incIoWriteCount(current_pid);
			if(reg_2==0){
